...
from parallax import create_parallax_menu
import sys
...
def main():
    ...
    # Botones del menú (empiezan vacíos)
    btn_exit = None
    btn_play = None
    btn_opc = None
    btn_creditos = None
    ...
    sfx_medidor_ok = pygame.mixer.Sound("assets/audio/sfx/cuota_ok.wav")
    sfx_medidor_ok.set_volume(0.7)  # ajusta a tu gusto

    ventana = pygame.display.set_mode((constantes.ANCHO_VENTANA, constantes.ALTO_VENTANA))
    pygame.display.set_caption("Krab's adventure")
    from parallax import create_parallax_menu

    menu_parallax = create_parallax_menu()
    try:
    ...
    # ==============================
    # PARALLAX DEL MENU (UNA SOLA VEZ)
    # ==============================
    menu_parallax = create_parallax_menu()
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if estado == ESTADO_OPC and nuevo_slider is not None:
                ...
                # 1. ACTUALIZAR APARIENCIA (Solo si existe)
                if 'btn_exit' in locals() and btn_exit is not None:
                    btn_exit.update(pygame.mouse.get_pos(), pygame.mouse.get_pressed()[0])

                # 2. MANEJO DE EVENTOS (Solo ocurren cuando hay un clic real o tecla)
                if event is not None:
                    # Clic en botón Atrás
                    if 'btn_exit' in locals() and btn_exit is not None:
                        if btn_exit.clicked(event):
                            print(f"[DEBUG] Botón 'Atrás' presionado")
                            estado = ESTADO_MENU
                            menu_krab = MenuKrab(midbottom=KRAB_MENU_POS, scale=KRAB_MENU_SCALE)
                            # Salimos del elif para que no procese nada más este frame
                            continue
                    ...
                    if now_ms - last_select_time >= SELECT_COOLDOWN_MS and not select_lock:
                        ...
                        if choice in ("male", "female"):
                            ...
                            # Crear al jugador
                            jugador = Personaje(100, 670, gender=selected_gender)
                ...
                # 2. MANEJO DE EVENTOS (Solo si ocurre un clic o tecla real)
                if event is not None:
                    # Botón Atrás: Solo funciona si haces clic de verdad
                    if 'btn_exit' in locals() and btn_exit is not None:
                        if btn_exit.clicked(event):
                            print(f"[DEBUG] Regresando a Selección de Personaje")
                            estado = ESTADO_SELECT_PERSONAJE
                            select_lock = False
                            continue  # Saltamos al siguiente frame para limpiar la entrada
                    ...
                    if choice is not None:
                        # Configuración común para niveles 1, 2 y 3
                        if choice in (1, 2, 3):
                            nivel_actual = choice
                ...
                if event is not None:
                    ...
                    if 'btn_exit' in locals() and btn_exit is not None:
                        if btn_exit.clicked(event):
                            print(f"[DEBUG] Regresando a Selección de Nivel")
                            estado = ESTADO_SELECT_NIVEL
                            continue

                            # Lógica de la UI de dificultad
        ...
        if estado == ESTADO_MENU:
            menu_parallax.update_by_camera(0.3)
            ...
            for item in list(items.sprites()):
                if item.tocar_jugador(jugador):
                    ...
                    if isinstance(item, (botella, bolsa, lamina, llanta, gustambo)):
                        ...
                        # ===== MEDIDOR (cuota) =====
                        estaba_completo = medidor.completado
                        medidor.agregar_basura(1)  # ✅ SOLO UNA VEZ
                        ahora_completo = medidor.completado

                        # 🔔 Sonido SOLO cuando cruza el umbral (70/90%), una sola vez
                        if (not estaba_completo) and ahora_completo and (not medidor._cuota_sonada):
                            musica.play_cuota_ok()  # ✅ aquí suena solo al completar cuota
                            medidor._cuota_sonada = True

                        # ===== VIDA EXTRA (cada X basuras) =====
                        trash_collected += 1
                        if trash_collected >= trash_threshold:
                            trash_collected -= trash_threshold

                            # ❤️ SOLO dar vida (y sonido) si NO está al máximo
                            if jugador.vida_actual < jugador.vida_maxima:
                                musica.sfx(name="1up", volume=0.9)
                                jugador.vida_actual += 1
        ...
        # ================================
            ...
            if 'btn_exit' in locals() and btn_exit is not None:
                btn_exit.update(pygame.mouse.get_pos(), pygame.mouse.get_pressed()[0])
                # Lógica para volver al menú con el botón
                if event is not None and btn_exit.clicked(event):
                    estado = ESTADO_MENU
                    continue
            ...
            # Seleccionar las imágenes del idioma actual
            ...
            # Arrastre del slider
            if drag_volume:
                rel_x = pygame.mouse.get_pos()[0] - nuevo_slider.x
                nuevo_volumen = rel_x / nuevo_slider.width
                settings["volume"] = max(0, min(1, nuevo_volumen))
                pygame.mixer.music.set_volume(settings["volume"])

            # ================================
            # 4) TÍTULO “Selecciona tu idioma”
            # ================================
            ...
            # Aura y Dibujo de botones (Español)
            if settings["language"] == "es":
                pygame.draw.rect(ventana, (255, 255, 160), btnIDIOMA_es.inflate(18, 18), border_radius=12)

            hover_es = btnIDIOMA_es.collidepoint(pygame.mouse.get_pos())
            if hover_es:
                pygame.draw.rect(ventana, (255, 255, 180), btnIDIOMA_es.inflate(12, 12), border_radius=12)
            ...
            texto_es = get_font(constantes.FONT_HUD).render(I18N[lang]["spanish"], True, (255, 255, 255))
            ventana.blit(texto_es, (btnIDIOMA_es.centerx - texto_es.get_width() // 2 + 18, btnIDIOMA_es.y + 24))

            # Aura y Dibujo de botones (Inglés)
            if settings["language"] == "en":
                pygame.draw.rect(ventana, (255, 255, 160), btnIDIOMA_en.inflate(18, 18), border_radius=12)

            hover_en = btnIDIOMA_en.collidepoint(pygame.mouse.get_pos())
            if hover_en:
                pygame.draw.rect(ventana, (255, 255, 180), btnIDIOMA_en.inflate(12, 12), border_radius=12)
            ...
            texto_en = get_font(constantes.FONT_HUD).render(I18N[lang]["english"], True, (255, 255, 255))
            ventana.blit(texto_en, (btnIDIOMA_en.centerx - texto_en.get_width() // 2 + 18, btnIDIOMA_en.y + 24))

            # 9) Clic de Lenguaje
            if click:
                if btnIDIOMA_es.collidepoint(pygame.mouse.get_pos()):
                    settings["language"] = "es"
                    mensaje_idioma = MensajeTemporal("Idioma cambiado")
                elif btnIDIOMA_en.collidepoint(pygame.mouse.get_pos()):
                    settings["language"] = "en"
            ...
            if 'select_ui' in locals() and select_ui: select_ui.draw(ventana)
            ...
            if 'level_select_ui' in locals() and level_select_ui: level_select_ui.draw(ventana)
            ...
            if 'diff_ui' in locals() and diff_ui: diff_ui.draw(ventana)
