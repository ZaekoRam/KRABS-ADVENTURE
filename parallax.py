...
def create_parallax_menu():
    base = Path(__file__).resolve().parent / "assets" / "menu"

    fondo = pygame.image.load(str(base / "fondo.png")).convert()
    nubes = pygame.image.load(str(base / "nubes.png")).convert_alpha()
    oceano = pygame.image.load(str(base / "oceano.png")).convert_alpha()
    palmera = pygame.image.load(str(base / "palmera.png")).convert_alpha()

    # Escalar a alto de pantalla
    def scale(img):
        new_w = int(img.get_width() * (SCREEN_H / img.get_height()))
        return pygame.transform.scale(img, (new_w, SCREEN_H))

    layers = [
        ParallaxLayer(scale(fondo),   0.0),   # fijo
        ParallaxLayer(scale(nubes),   0.02),  # suave
        ParallaxLayer(scale(oceano),  0.06),  # medio
        ParallaxLayer(scale(palmera), 0.12),  # primer plano
    ]
